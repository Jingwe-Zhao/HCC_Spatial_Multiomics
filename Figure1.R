...
samples_dir
## define sample id
sample_names <- c("P1", "P2", "P3", "P4", "N1", "N3")

##Load 10x Visium  data and create Seurat object
...
##Fig1A----
col = c('1'="#7fc97f", '2'="#beaed4", '3'="#fdc086", '4'="#386cb0", '5'="#f0027f", '6'="#a34e3b", '7'="#666666",
        '8'="#1b9e77", '9'="#d95f02", '10'="#6c67a5", '11'="#d01b2a", '12'="#43acde", '13'="#efbd25")
...
##Fig1C-E----
#H&E + Clusters
h1<-SpatialPlot(ST,  pt.size.factor =0,images = 'P1')+NoLegend()
c1<-SpatialPlot(ST, group.by = 'clusters',stroke = 0.15, pt.size.factor =1.4,images = 'P1',image.alpha = 0,cols = col)+NoLegend()

h2<-SpatialPlot(ST,  pt.size.factor =0,images = 'P2')+NoLegend()
c2<-SpatialPlot(ST, group.by = 'clusters',stroke = 0.15, pt.size.factor =2.3,images = 'P2',image.alpha = 0,cols = col)+NoLegend()

h3<-SpatialPlot(ST,  pt.size.factor =0,images = 'P3')+NoLegend()
c3<-SpatialPlot(ST, group.by = 'clusters',stroke = 0.15, pt.size.factor =1.6,images = 'P3',image.alpha = 0,cols = col)+NoLegend()

h4<-SpatialPlot(ST,  pt.size.factor =0,images = 'P4')+NoLegend()
c4<-SpatialPlot(ST, group.by = 'clusters',stroke = 0.15, pt.size.factor =1.5,images = 'P4',image.alpha = 0,cols = col)+NoLegend()

h5<-SpatialPlot(ST,  pt.size.factor =0,images = 'N1')+NoLegend()
c5<-SpatialPlot(ST, group.by = 'clusters',stroke = 0.15, pt.size.factor =1.6,images = 'N1',image.alpha = 0,cols = col)+NoLegend()

h6<-SpatialPlot(ST, pt.size.factor =0,images = 'N3')+NoLegend()
c6<-SpatialPlot(ST, group.by = 'clusters',stroke = 0.15, pt.size.factor =1.3,images = 'N3',image.alpha = 0,cols = col)+NoLegend()
...
#plot cell ratio across samples
...
#plot cluster and cell type correlation
...
#Cell type Spatial co-localization analysis
...
#load Spatial co-localization result
...
##C2 Cluster4 tumor
ST@meta.data$Region[ST@meta.data$cluster %in% c('4') & ST@meta.data$orig.ident %in% c('P1')] <- "T"
ST@meta.data$Region[ST@meta.data$cluster %in% c('3','5')] <- "TC"
##C2Cluster7 TR
ST@meta.data$Region[ST@meta.data$cluster %in% c('7')& ST@meta.data$orig.ident %in% c('P3')] <- "TR"

col2<-c('PT'='#FDB462','TC'='#1965B0','T'='#DC050C','TR'='#B17BA6')

Idents(ST)<-ST@meta.data$Region

#plot region division result
plot_gene = function (cluster){
  p1 = SpatialPlot(ST, group.by = cluster,stroke = 0.15, pt.size.factor =1.4,images = 'P1',image.alpha = 0,cols = col2)
  p2 = SpatialPlot(ST, group.by = cluster,stroke = 0.15,pt.size.factor = 2.3,images = 'P2',image.alpha = 0,cols = col2)
  p3 = SpatialPlot(ST, group.by = cluster,stroke = 0.15,pt.size.factor = 1.6,images = 'P3',image.alpha = 0,cols = col2)
  p4 = SpatialPlot(ST, group.by = cluster,stroke = 0.15,pt.size.factor = 1.5,images = 'P4',image.alpha = 0,cols = col2)
