###Figure7: CAF subtypes specific metabolite analysis--------------
library(MetaboDiff)
library(Seurat)
library(Matrix)
library(ggpubr)
...
HCC@meta.data$REGION <- ifelse(HCC@meta.data$clusters %in% c('3','5'), "TC", "Other")
...
mydem=met@metadata[["ttest_REGION_TC_vs_Other"]]
...
##Fig7D----
#RCTD-MSI Corr
load('~/DATA/luo/zhongnan/SM/Corr/P1_cell_msi_cor.rdata')
p1Corr<-cor_results
load('~/DATA/luo/zhongnan/SM/Corr/P2_cell_msi_cor.rdata')
p2Corr<-cor_results
load('~/DATA/luo/zhongnan/SM/Corr/P3_cell_msi_cor.rdata')
p3Corr<-cor_results
load('~/DATA/luo/zhongnan/SM/Corr/P4_cell_msi_cor.rdata')
p4Corr<-cor_results


#correlation_list
correlation_list <- list(p1Corr, p2Corr, p3Corr, p4Corr)
#Calculate the mean
