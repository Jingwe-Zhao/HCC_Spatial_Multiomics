...
##Fig2A----
load('/home/zhaojingwei/DATA/luo/zhongnan/SM/HCC_SM.rdata')

col = c('1'="#7fc97f", '2'="#beaed4", '3'="#fdc086", '4'="#386cb0", '5'="#f0027f", '6'="#a34e3b", '7'="#666666",
        '8'="#1b9e77", '9'="#d95f02", '10'="#6c67a5", '11'="#d01b2a", '12'="#43acde", '13'="#efbd25",'14'='#45306f','15'='#ba5074')


DimPlot(SM,group.by = 'SM_clusters',cols = col)

##Fig2B----
#H&E + SM_Clusters
h1<-SpatialPlot(SM,  pt.size.factor =0,images = 'P1')+NoLegend()
c1<-SpatialPlot(SM, group.by = 'clusters',stroke = 0.15, pt.size.factor =1.4,images = 'P1',image.alpha = 0,cols = col)+NoLegend()

h2<-SpatialPlot(SM,  pt.size.factor =0,images = 'P2')+NoLegend()
c2<-SpatialPlot(SM, group.by = 'clusters',stroke = 0.15, pt.size.factor =2.3,images = 'P2',image.alpha = 0,cols = col)+NoLegend()

h3<-SpatialPlot(SM,  pt.size.factor =0,images = 'P3')+NoLegend()
c3<-SpatialPlot(SM, group.by = 'clusters',stroke = 0.15, pt.size.factor =1.6,images = 'P3',image.alpha = 0,cols = col)+NoLegend()

h4<-SpatialPlot(SM,  pt.size.factor =0,images = 'P4')+NoLegend()
c4<-SpatialPlot(SM, group.by = 'clusters',stroke = 0.15, pt.size.factor =1.5,images = 'P4',image.alpha = 0,cols = col)+NoLegend()


p<-ggarrange(h1,h2,h3,h4,c1,c2,c3,c4,ncol = 4,nrow = 2)
p
ggsave(plot = p,filename = "/home/zhaojingwei/DATA/luo/zhongnan/SM/Fig/Fig2/Fig2B.pdf",width = 12,height = 5.3)


##Fig2C----
DoHeatmap(SM,features = topDEM,group.by = 'SM_clusters',group.colors=col)

##Fig2D----
#Correlation between SM_Clusters
corrplot(df_cor, 
         method = "pie",   
         type = "upper",
         col = colorRampPalette(c("#7780b1","white","#eb8c94"))(100),
         tl.cex=0.7, 
         tl.col ="black", 
         tl.offset=0.5, 
         tl.srt=90, 
...
##Fig2e-f----
#MSI_Cluster metabolites Distribution
...
if (sel_cla == "Class"){
  p <- ggplot(data=SuperClass_count, aes(x=reorder(Class, -Count), y=Count, fill=Region)) +
    ...
    scale_fill_manual(values=c('#E69F00','#999999'), labels=c('Total',lg_label)) +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1, size = 15.3, color = "black"), 
